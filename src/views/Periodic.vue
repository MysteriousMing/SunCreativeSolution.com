...
export default {
  ...
  methods: {
    drawSimpleLine () {
      ...
      this.windowWith = this.container.width
      this.windowHinnerHeight = this.container.height
      let threeExample = document.querySelector('#threeExample')
    ...
    init () {
      this.camera = new THREE.PerspectiveCamera(40, this.container.width / this.container.height, 1, 10000)
      ...
      for (let i = 0; i < this.table.length; i += 5) {
        ...
        let object = new THREE.Object3D(element)
      ...
      // this.renderer = new THREE.Object3D()
      ...
      this.controls = new THREE.EventDispatcher(this.camera, this.renderer.domElement)
...
.page-dashboard .element:hover {
	box-shadow: 0px 0px 12px rgba(0,255,255,0.75);
	border: 1px solid rgba(127,255,255,0.75);
}
.page-dashboard 	.element .number {
		position: absolute;
		top: 20px;
		right: 20px;
		font-size: 12px;
...
.page-dashboard 	.element .details {
		position: absolute;
		bottom: 15px;
		left: 0px;
		right: 0px;
